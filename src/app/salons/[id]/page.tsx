...
const card: any = {
  background: 'white', borderRadius: 12, border: '1px solid #DBDBDB', padding: '16px', marginBottom: 12,
}

const sectionTitle: any = {
  fontSize: 13, fontWeight: 700, color: '#262626', paddingBottom: 10, marginBottom: 12, borderBottom: '1px solid #DBDBDB',
}

const inputStyle: any = {
  width: '100%', border: '1.5px solid #DBDBDB', borderRadius: 10, padding: '10px 14px', fontSize: 13,
  fontFamily: 'inherit', outline: 'none', color: '#111', background: '#FAFAFA',
}
...
export default function SalonDetailPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      setUser(data.user)
      if (data.user) {
        const { data: prof } = await supabase.from('profiles').select('full_name, display_name, username').eq('id', data.user.id).single()
        setUserProfile(prof)
      }
    })
    fetchSalonData()
  ...
  const fetchSalonData = async () => {
    ...
    if (currentUser) {
      const { data: blockData } = await supabase.from('blocks').select('id').eq('blocked_id', currentUser.id).eq('blocked_salon_id', id).maybeSingle()
      setIsBlocked(!!blockData)
      const { count } = await supabase.from('reservations').select('id', { count: 'exact', head: true }).eq('user_id', currentUser.id).eq('salon_id', id).eq('status', 'completed')
  ...
  const loadAvailability = async (stylistId: string | null) => {
    ...
    if (stylistId) {
      ...
      const { data: resData } = await supabase.from('reservations').select('*, menus(duration)').eq('stylist_id', stylistId).not('status', 'in', '("cancelled","expired")').gte('reserved_at', now.toISOString()).lte('reserved_at', future.toISOString())
      ...
      const { data: resData } = await supabase.from('reservations').select('*, menus(duration)').eq('salon_id', id).is('stylist_id', null).not('status', 'in', '("cancelled","expired")').gte('reserved_at', now.toISOString()).lte('reserved_at', future.toISOString())
      setReservations(resData || [])
    }
  ...
  const salonBusinessHours = salon?.business_hours_start && salon?.business_hours_end
    ? { start: salon.business_hours_start, end: salon.business_hours_end } : null
  ...
  const maxBookingDate = useMemo(() => {
    const d = new Date(); d.setDate(d.getDate() + 60); d.setHours(23, 59, 59, 999); return d
  }, [])

  const maxMonth = useMemo(() => {
    const d = new Date(); d.setDate(d.getDate() + 60); return new Date(d.getFullYear(), d.getMonth(), 1)
  }, [])
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let i = 0; i < firstDay; i++) days.push({ date: '', available: false, isPast: false, isEmpty: true })
    for (let d = 1; d <= daysInMonth; d++) {
      const dateStr = `${year}-${(month + 1).toString().padStart(2, '0')}-${d.toString().padStart(2, '0')}`
      const date = new Date(dateStr + 'T00:00:00+09:00')
      const isPast = date < today; const isTooFar = date > maxBookingDate
  ...
  const makeReservation = async () => {
    ...
    if (selectedMenu?.is_first_visit) {
      const { count } = await supabase.from('reservations').select('id', { count: 'exact', head: true }).eq('user_id', user.id).eq('salon_id', id).eq('status', 'completed')
      if ((count ?? 0) > 0) { alert('このメニューは初回限定です。2回目以降のご来店には適用できません。'); return }
    }

    setBookingLoading(true)
    const slotDate = new Date(`${selectedDate}T${selectedTime}:00+09:00`)
    const now = new Date()
    if ((slotDate.getTime() - now.getTime()) / (1000 * 60 * 60) < deadlineHours) {
    ...
    const { data: userDup } = await supabase.from('reservations').select('id').eq('user_id', user.id).eq('salon_id', id).eq('reserved_at', reservedAt).in('status', ['pending', 'confirmed']).maybeSingle()
    if (userDup) { alert('この日時はすでに予約済みです。'); setBookingLoading(false); return }

    if (selectedStylist?.id) {
      const endTime = new Date(new Date(reservedAt).getTime() + selectedMenu.duration * 60 * 1000).toISOString()
      const { data: stylistDup } = await supabase.from('reservations').select('id, reserved_at, menus(duration)').eq('stylist_id', selectedStylist.id).in('status', ['pending', 'confirmed']).gte('reserved_at', reservedAt).lt('reserved_at', endTime).maybeSingle()
      if (stylistDup) { alert('この時間帯はスタイリストの予約が埋まっています。別の時間をお選びください。'); setBookingLoading(false); return }
    }
    ...
    if (requestImageFile) {
      ...
      const { error: uploadError } = await supabase.storage.from('salon-images').upload(fileName, requestImageFile, { upsert: true })
      if (!uploadError) {
    ...
    const { data: newRes, error } = await supabase.from('reservations').insert({
      user_id: user.id, salon_id: id, menu_id: selectedMenu.id,
      stylist_id: selectedStylist?.id || null, reserved_at: reservedAt,
      request_image_url: requestImageUrl,
    }).select().single()
    if (error || !newRes) { alert('予約失敗: ' + (error?.message || '')); setBookingLoading(false); return }
    ...
    await sendEmail(user.email!, ...Object.values(emailTemplates.reservationPending(salon.name, selectedMenu.name, dateStr)) as [string, string])
    const { data: ownerProfile } = await supabase.from('profiles').select('username').eq('id', salon.owner_id).single()
    if (ownerProfile?.username) {
      await sendEmail(ownerProfile.username, ...Object.values(emailTemplates.newReservation(user.email!, selectedMenu.name, dateStr)) as [string, string])
    }
      ...
      {(() => {
        ...
        return (
          <div className="sp-gallery-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gridTemplateRows: '150px 100px', gap: 2, maxHeight: 252, overflow: 'hidden', background: '#000' }}>
            ...
            {[1, 2, 3, 4].map(i => (
              images[i] ? <img key={i} src={images[i]} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} /> : <div key={i} style={{ background: '#F0E6F5' }} />
            ))}
          </div>
        )
      })()}
      ...
      <div className="sp-salon-layout" style={{ display: 'grid', gridTemplateColumns: '1fr 240px', gap: 16, maxWidth: '980px', margin: '0 auto', padding: '16px 16px 40px' }}>
        ...
        <div style={{ minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ marginBottom: 6 }}>
                  <span style={{ display: 'inline-block', fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 100, background: 'linear-gradient(135deg,#FFF0F5,#F5F0FF)', color: '#833AB4', border: '1px solid #E1306C33' }}>{salon.genre || 'ヘアサロン'}</span>
                </div>
                ...
                <div style={{ fontSize: 12, color: '#737373', lineHeight: 2.0, display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: '#BDBDBD', flexShrink: 0 }}>住所</span>
                    <span>{salon.area}　{salon.address}</span>
                  </div>
                  {salon.nearest_station && <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}><span style={{ fontSize: 10, fontWeight: 700, color: '#BDBDBD', flexShrink: 0 }}>最寄駅</span><span>{salon.nearest_station}駅近く</span></div>}
                  {salon.phone && <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}><span style={{ fontSize: 10, fontWeight: 700, color: '#BDBDBD', flexShrink: 0 }}>TEL</span><span>{salon.phone}</span></div>}
                </div>
              </div>
              <button onClick={() => { setInfoTab('booking'); setStep(1) }} style={{ background: grad, color: 'white', border: 'none', padding: '10px 20px', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', flexShrink: 0 }}>空席確認・予約する</button>
            </div>
          </div>

          <div style={{ background: 'white', border: '1px solid #DBDBDB', borderTop: 'none', display: 'flex', overflowX: 'auto', marginBottom: 0 }}>
            {([{ key: 'info', label: 'サロン情報' }, { key: 'styles', label: 'スタイル' }, { key: 'booking', label: '予約する' }, { key: 'stylists', label: 'スタイリスト' }] as const).map(t => (
              <button key={t.key} onClick={() => { setInfoTab(t.key); if (t.key === 'booking') setStep(1) }}
                style={{ padding: '11px 18px', fontSize: 13, fontWeight: 700, border: 'none', borderBottom: infoTab === t.key ? '2px solid #E1306C' : '2px solid transparent', background: 'none', cursor: 'pointer', fontFamily: 'inherit', color: infoTab === t.key ? '#111' : '#737373', whiteSpace: 'nowrap', transition: 'all 0.15s' }}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          <div style={{ ...card, borderRadius: '0 0 12px 12px', borderTop: 'none' }}>
              ...
              <div>
                {salon.description && (
                  <div style={{ marginBottom: 24 }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: '#E1306C', lineHeight: 1.6, marginBottom: 10 }}>{salon.description.split('\n')[0]}</div>
                    <p style={{ fontSize: 13, color: '#444', lineHeight: 2.0 }}>{salon.description.split('\n').slice(1).join('\n') || salon.description}</p>
                  </div>
                )}
                ...
                {menus.length > 0 && (
                  <div style={{ marginBottom: 24 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {menus.map(menu => (
                        <div key={menu.id} style={{ border: '1px solid #DBDBDB', borderRadius: 10, overflow: 'hidden', background: 'white' }}>
                          {menu.image_url && <img src={menu.image_url} alt={menu.name} style={{ width: '100%', height: 160, objectFit: 'cover', display: 'block' }} />}
                          <div style={{ padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                            <div style={{ flex: 1 }}>
                              ...
                              {menu.description && <div style={{ fontSize: 12, color: '#737373', lineHeight: 1.6, marginBottom: 4 }}>{menu.description}</div>}
                            ...
                            <div style={{ flexShrink: 0, textAlign: 'right' }}>
                              ...
                              <button onClick={() => { setSelectedMenu(menu); setInfoTab('booking'); setStep(2) }} style={{ marginTop: 6, background: grad, color: 'white', border: 'none', padding: '5px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' as const }}>このメニューで予約</button>
                            </div>
                          </div>
                        </div>
                      ))}
                      ...
                      <tr style={{ borderBottom: '1px solid #F2F2F2' }}>
                        ...
                        <td style={{ padding: '9px 0' }}>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                            {salon.payment_methods.map((m: string) => <span key={m} style={{ fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 100, background: '#F5F0FF', color: '#833AB4', border: '1px solid #E1D5F5' }}>{m}</span>)}
                            {salon.payment_other && <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 100, background: '#F5F5F5', color: '#737373', border: '1px solid #DBDBDB' }}>{salon.payment_other}</span>}
                          </div>
                        </td>
                      </tr>
                    )}
                  ...
                  {salon.address && (
                    <div style={{ marginTop: 16 }}>
                      ...
                      <div style={{ borderRadius: 10, overflow: 'hidden', border: '1px solid #DBDBDB' }}>
                        <iframe src={`https://maps.google.com/maps?q=${encodeURIComponent((salon.area || '') + ' ' + (salon.address || ''))}&output=embed&hl=ja`} width="100%" height="240" style={{ border: 'none', display: 'block' }} loading="lazy" title="アクセスマップ" />
                      </div>
                      <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent((salon.area || '') + ' ' + (salon.address || ''))}`} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 6, fontSize: 11, color: '#5851DB', textDecoration: 'none', fontWeight: 700 }}>Google マップで開く →</a>
                    </div>
                  )}
                </div>
              </div>
                      ...
                      return (
                        <div key={s.id} style={{ marginBottom: 24 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                            ...
                            <div><div style={{ fontSize: 13, fontWeight: 700, color: '#111' }}>{s.name}</div>{s.role && <div style={{ fontSize: 11, ...gradText }}>{s.role}</div>}</div>
                          </div>
                          <div className="sp-photo-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6 }}>
                            {photos.map(photo => <img key={photo.id} src={photo.image_url} alt="" style={{ width: '100%', aspectRatio: '1', objectFit: 'cover', borderRadius: 8 }} />)}
                          </div>
                        </div>
                      )
                    })}
                  ...
                  : stylists.map(s => (
                    <div key={s.id} style={{ display: 'flex', gap: 14, padding: '14px 0', borderBottom: '1px solid #DBDBDB' }}>
                      <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'linear-gradient(135deg,#FBE0EC,#EED9F7)', overflow: 'hidden', flexShrink: 0 }}>
                        {s.image_url ? <img src={s.image_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, fontWeight: 700, ...gradText }}>{s.name?.[0]}</div>}
                      </div>
                      <div>
            ...
            {infoTab === 'booking' && (
              <div>
                {isBlocked && (
                  <div style={{ background: '#FFEBEE', border: '1.5px solid #FFCDD2', borderRadius: 10, padding: '12px 16px', marginBottom: 16, fontSize: 13, color: '#C62828', fontWeight: 700 }}>このサロンへの予約はご利用いただけません。</div>
                )}
                {!user && (
                  <div style={{ background: 'linear-gradient(135deg,#FFF0F5,#F5F0FF)', border: '1.5px solid #E1D5F5', borderRadius: 12, padding: '16px', marginBottom: 16 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#737373', lineHeight: 1.8, marginBottom: 12 }}>✓ 予約・キャンセルをかんたん管理<br />✓ 初回限定クーポンが使える<br />✓ 来店後にXやインスタでシェア</div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => router.push(`/auth?redirect=/salons/${id}`)} style={{ flex: 1, background: 'linear-gradient(45deg,#F77737,#E1306C,#833AB4,#5851DB)', color: 'white', border: 'none', padding: '10px 0', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>無料で会員登録</button>
                      <button onClick={() => router.push(`/auth?redirect=/salons/${id}`)} style={{ flex: 1, background: 'white', color: '#737373', border: '1.5px solid #DBDBDB', padding: '10px 0', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>ログイン</button>
                    </div>
                  </div>
                )}

                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24 }}>
                  {[{ n: 1, label: 'メニュー' }, { n: 2, label: 'スタイリスト' }, { n: 3, label: '日時' }, { n: 4, label: '確認' }].map((s, i) => (
                    <div key={s.n} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                        <div style={{ width: 30, height: 30, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, background: step === s.n ? grad : step > s.n ? '#F0E6F5' : '#F2F2F2', color: step === s.n ? 'white' : step > s.n ? '#833AB4' : '#BDBDBD' }}>{s.n}</div>
                  ...
                  <div>
                    ...
                    {menus.length === 0
                      ...
                      : menus.map(menu => {
                          ...
                          return (
                            <div key={menu.id} onClick={() => canSelect && setSelectedMenu(menu)}
                              style={{ padding: 14, border: selectedMenu?.id === menu.id ? '2px solid #E1306C' : isLocked ? '1.5px solid #EEEEEE' : '1.5px solid #DBDBDB', borderRadius: 10, cursor: canSelect ? 'pointer' : 'not-allowed', marginBottom: 8, background: selectedMenu?.id === menu.id ? '#FFF0F5' : isLocked ? '#F8F8F8' : 'white', opacity: isLocked ? 0.55 : 1, transition: 'all 0.15s', position: 'relative' as any }}>
                              {isFirstOnly && (
                                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
                                  <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 100, background: isLocked ? '#E0E0E0' : 'linear-gradient(45deg,#F77737,#E1306C)', color: 'white' }}>初回限定</span>
                                  {isLocked && <span style={{ fontSize: 10, color: '#BDBDBD' }}>※ 2回目以降のご来店には適用できません</span>}
                                  {!isLocked && isFirstVisit && <span style={{ fontSize: 10, color: '#E1306C' }}>✓ あなたは初回限定対象です</span>}
                                  {!isLocked && isFirstVisit === null && <button onClick={() => router.push(`/auth?redirect=/salons/${id}`)} style={{ fontSize: 10, fontWeight: 700, color: 'white', background: 'linear-gradient(45deg,#F77737,#E1306C)', border: 'none', padding: '2px 10px', borderRadius: 100, cursor: 'pointer', fontFamily: 'inherit' }}>会員登録で使える →</button>}
                                </div>
                              )}
                    ...
                    <button onClick={() => selectedMenu && setStep(2)} disabled={!selectedMenu} style={{ width: '100%', marginTop: 8, background: selectedMenu ? grad : '#E0E0E0', color: 'white', border: 'none', padding: 13, borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: selectedMenu ? 'pointer' : 'not-allowed', fontFamily: 'inherit', opacity: selectedMenu ? 1 : 0.5 }}>次へ（スタイリストを選ぶ）→</button>
                  </div>
                )}
                ...
                {step === 2 && (
                  <div>
                    ...
                    <div onClick={() => handleStylistSelect(null)} style={{ padding: 14, border: '1.5px dashed #F0A0C0', borderRadius: 10, cursor: 'pointer', marginBottom: 10, background: '#FFF0F5', display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div><div style={{ fontSize: 14, fontWeight: 700, color: '#111' }}>指名なし（フリー枠）</div><div style={{ fontSize: 11, color: '#737373', marginTop: 2 }}>サロンにお任せします</div><div style={{ fontSize: 11, color: '#E1306C', marginTop: 2 }}>空き枠が最も多く取りやすいです</div></div>
                    </div>
                    {stylists.length > 0 && (
                      <>
                        <div style={{ fontSize: 11, fontWeight: 700, color: '#737373', marginBottom: 8 }}>スタイリストを指名する</div>
                        {stylists.map(s => (
                          <div key={s.id} onClick={() => handleStylistSelect(s)} style={{ padding: 14, border: '1.5px solid #DBDBDB', borderRadius: 10, cursor: 'pointer', marginBottom: 8, background: 'white', display: 'flex', alignItems: 'center', gap: 12 }}>
                            <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'linear-gradient(135deg,#FBE0EC,#EED9F7)', overflow: 'hidden', flexShrink: 0 }}>
                              {s.image_url ? <img src={s.image_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700, ...gradText }}>{s.name?.[0]}</div>}
                            </div>
                            <div style={{ flex: 1 }}>
                              <div style={{ fontSize: 14, fontWeight: 700, color: '#111' }}>{s.name}</div>
                      ...
                      <>
                        {nearestAvailableDate && !selectedDate && (
                          <div style={{ background: '#E8F5E9', border: '1px solid #A5D6A7', borderRadius: 10, padding: '10px 14px', marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <button onClick={() => setSelectedDate(nearestAvailableDate)} style={{ background: '#43A047', color: 'white', border: 'none', padding: '4px 12px', borderRadius: 100, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>この日を選ぶ</button>
                          </div>
                        )}
                        <div style={{ background: '#FAFAFA', borderRadius: 10, border: '1px solid #DBDBDB', padding: 14, marginBottom: 12 }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                            <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1))} style={{ width: 30, height: 30, borderRadius: '50%', border: '1px solid #DBDBDB', background: 'white', cursor: 'pointer', fontSize: 12, color: '#737373', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'inherit' }}>◀</button>
                            <span style={{ fontWeight: 700, fontSize: 14, color: '#111' }}>{currentMonth.getFullYear()}年{currentMonth.getMonth() + 1}月</span>
                            <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1))} disabled={currentMonth >= maxMonth} style={{ width: 30, height: 30, borderRadius: '50%', border: '1px solid #DBDBDB', background: 'white', cursor: currentMonth >= maxMonth ? 'not-allowed' : 'pointer', fontSize: 12, color: currentMonth >= maxMonth ? '#BDBDBD' : '#737373', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'inherit', opacity: currentMonth >= maxMonth ? 0.4 : 1 }}>▶</button>
                          </div>
                          <div style={{ display: 'flex', gap: 12, marginBottom: 8 }}>
                            <span style={{ fontSize: 10, color: '#737373', display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ display: 'inline-block', width: 14, height: 14, borderRadius: 3, background: '#E8F5E9', border: '1px solid #A5D6A7' }} />○ 空きあり</span>
                            <span style={{ fontSize: 10, color: '#737373', display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ display: 'inline-block', width: 14, height: 14, borderRadius: 3, background: '#F5F5F5' }} />× 受付不可</span>
                          </div>
                          <div className="sp-calendar-header" style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', marginBottom: 4 }}>
                            {DAY_NAMES.map((d, i) => <div key={d} style={{ textAlign: 'center', fontSize: 11, fontWeight: 700, padding: '4px 0', color: i === 0 ? '#E53935' : i === 6 ? '#5851DB' : '#737373' }}>{d}</div>)}
                          </div>
                            ...
                            {calendarDays.map((day, i) => (
                              <div key={i} style={{ aspectRatio: '1' }}>
                                {day.isEmpty ? <div /> : day.isPast ? (
                                  <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#BDBDBD' }}>{parseInt(day.date.split('-')[2])}</div>
                                ) : (
                                  <button onClick={() => { if (day.available) { setSelectedDate(day.date); setSelectedTime('') } }} disabled={!day.available}
                                    style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: 6, fontSize: 11, fontWeight: 700, border: 'none', cursor: day.available ? 'pointer' : 'not-allowed', fontFamily: 'inherit', background: selectedDate === day.date ? grad : day.available ? '#E8F5E9' : 'transparent', color: selectedDate === day.date ? 'white' : day.available ? '#2E7D32' : '#BDBDBD', outline: selectedDate === day.date ? 'none' : day.available ? '1px solid #A5D6A7' : 'none' }}>
                        ...
                        {selectedDate && (
                          <div style={{ background: '#FAFAFA', borderRadius: 10, border: '1px solid #DBDBDB', padding: 14, marginBottom: 12 }}>
                            <div style={{ fontSize: 13, fontWeight: 700, color: '#111', marginBottom: 10 }}>{selectedDate.replace(/-/g, '/')} の空き時間</div>
                            {allTimeSlots.length === 0 ? <div style={{ fontSize: 12, color: '#737373' }}>この日は受付可能な時間帯がありません</div> : (
                              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6 }}>
                                {allTimeSlots.map(slot => {
                                  const available = timeSlots.includes(slot); const isSelected = selectedTime === slot
                                  return (
                                    <button key={slot} onClick={() => available && setSelectedTime(slot)} disabled={!available}
                                      style={{ padding: '8px 0', borderRadius: 8, fontSize: 12, fontWeight: 700, fontFamily: 'inherit', border: 'none', cursor: available ? 'pointer' : 'not-allowed', background: isSelected ? grad : available ? '#E8F5E9' : '#F5F5F5', color: isSelected ? 'white' : available ? '#2E7D32' : '#BDBDBD', outline: available && !isSelected ? '1px solid #A5D6A7' : 'none' }}>
                                      {slot}
                                    </button>
                                  )
                                })}
                        ...
                        <button onClick={() => selectedDate && selectedTime && setStep(4)} disabled={!selectedDate || !selectedTime} style={{ width: '100%', background: selectedDate && selectedTime ? grad : '#E0E0E0', color: 'white', border: 'none', padding: 13, borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: selectedDate && selectedTime ? 'pointer' : 'not-allowed', fontFamily: 'inherit' }}>次へ（予約内容を確認）→</button>
                      </>
                    )}
                  </div>
                )}
                ...
                {step === 4 && (
                  <div>
                    ...
                    <div style={{ background: 'white', border: '1px solid #DBDBDB', borderRadius: 10, padding: 14, marginBottom: 12 }}>
                      ...
                      <div style={{ fontSize: 11, color: '#737373', lineHeight: 1.7, marginBottom: 10 }}>参考にしたいヘアスタイルや仕上がりイメージの写真を添付できます。<br /><span style={{ color: '#E1306C', fontWeight: 700 }}>※ ご希望に必ずしもお応えできるとは限りません。</span></div>
                      {requestImagePreview ? (
                        <div style={{ position: 'relative', display: 'inline-block', marginBottom: 8 }}>
                          ...
                          <button onClick={() => { setRequestImageFile(null); setRequestImagePreview('') }} style={{ position: 'absolute', top: 4, right: 4, background: 'rgba(0,0,0,0.6)', color: 'white', border: 'none', width: 22, height: 22, borderRadius: '50%', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'inherit' }}>×</button>
                        </div>
                      ) : (
                        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, border: '1.5px dashed #DBDBDB', background: '#FAFAFA', color: '#737373', padding: '8px 16px', borderRadius: 10, cursor: 'pointer' }}>
                          📷 写真を選ぶ
                          <input type="file" accept="image/*" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) { setRequestImageFile(f); setRequestImagePreview(URL.createObjectURL(f)) } }} />
                        </label>
                      )}
                    </div>

                    {/* 事前決済案内（サロンがuse_prepayment=trueの場合） */}
                    {salon.use_prepayment && (
                      <div style={{ background: 'linear-gradient(135deg,#F0F4FF,#F5F0FF)', border: '1.5px solid #C5CAE9', borderRadius: 10, padding: '12px 14px', marginBottom: 14 }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: '#3949AB', marginBottom: 6 }}>💳 事前カード決済について</div>
                        <div style={{ fontSize: 11, color: '#444', lineHeight: 1.9 }}>
                          ・予約申請時にカード情報を入力します（仮押さえのみ）。<br />
                          ・サロンが承認した時点で引き落とし確定となります。<br />
                          ・3日以内に承認がない場合、予約・決済は自動キャンセルされます。<br />
                          ・<span style={{ fontWeight: 700 }}>施術時に追加メニューや商品を購入した場合は、サロン指定の支払い方法での現地払いとなります。</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div style={{ background: '#FFF0F5', border: '1px solid #F0A0C0', borderRadius: 10, padding: '10px 14px', marginBottom: 14 }}>
                      ...
                      <div style={{ fontSize: 11, color: '#C62828', lineHeight: 1.9 }}>
                        ・<span style={{ fontWeight: 700 }}>承認待ちの間</span>はマイページからキャンセルできます。<br />
                    ...
                    <button onClick={makeReservation} disabled={bookingLoading || !user}
                      ...
                      {bookingLoading ? '処理中...' : user ? (salon.use_prepayment ? '💳 カード決済して予約申請する' : '予約を申請する') : 'ログインが必要です'}
                    </button>
                  </div>
                )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={card}>
            {user ? (
              <>
                <div style={{ fontSize: 11, color: '#737373', marginBottom: 10, paddingBottom: 8, borderBottom: '1px solid #DBDBDB' }}>こんにちは、<strong style={{ color: '#111' }}>{displayName}</strong>さん</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg,#FBE0EC,#EED9F7)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0, fontWeight: 700, ...gradText }}>{displayName?.[0]?.toUpperCase() || '?'}</div>
                  <div><div style={{ fontSize: 13, fontWeight: 700, color: '#111' }}>{displayName}</div><div style={{ fontSize: 11, color: '#737373' }}>一般ユーザー</div></div>
                </div>
              </>
            ) : (
              <div style={{ fontSize: 12, color: '#737373', marginBottom: 12, textAlign: 'center' }}>ログインしていません</div>
            )}
            <button onClick={() => { setInfoTab('booking'); setStep(1) }} style={{ width: '100%', background: grad, color: 'white', border: 'none', padding: '10px 0', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', marginBottom: 8 }}>空席確認・予約する</button>
            {!user && <button onClick={() => router.push(`/auth?redirect=/salons/${id}`)} style={{ width: '100%', background: 'none', border: '1px solid #DBDBDB', padding: '8px 0', borderRadius: 8, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', color: '#737373' }}>ログイン / 新規登録</button>}
          </div>
          {user && (
            <div style={card}>
              ...
              {rightLinks.map(link => (
                <Link key={link.label} href={link.href} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#262626', textDecoration: 'none', padding: '8px 0', borderBottom: '1px solid #F2F2F2' }}>
                  <span style={{ fontSize: 10, color: '#E1306C' }}>▶</span>{link.label}
                </Link>
              ))}
              <button onClick={async () => { await supabase.auth.signOut(); router.push('/') }} style={{ width: '100%', marginTop: 8, background: 'none', border: '1px solid #DBDBDB', padding: '7px 0', borderRadius: 8, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', color: '#737373' }}>ログアウト</button>
            </div>
          )}
          {recentSalons.filter(s => s.id !== id).length > 0 && (
            <div style={card}>
              ...
              {recentSalons.filter(s => s.id !== id).map(s => (
                <Link key={s.id} href={`/salons/${s.id}`} style={{ display: 'block', padding: '7px 0', borderBottom: '1px solid #F2F2F2', textDecoration: 'none' }}>
