      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          {activeReservations.length === 0 ? (
            ...
            <div key={res.id} className="bg-white rounded-lg shadow p-4 mb-3">
              <div className="flex justify-between items-start mb-2">
                <span className={`text-xs px-2 py-1 rounded-full font-bold ${statusConfig[res.status].color}`}>
              ...
              <p className="text-sm text-pink-600 font-bold">¥{res.menus?.price?.toLocaleString()}</p>
              ...
              {res.status === 'pending' && (
                <div className="mt-3 p-3 bg-yellow-50 rounded-lg">
                  ...
                  <button onClick={() => cancelReservation(res.id)}
                    className="mt-2 text-xs text-red-500 border border-red-200 px-3 py-1 rounded">
                    予約をキャンセルする
                  </button>
                </div>
              )}
              ...
              {res.status === 'confirmed' && (
                <div className="mt-3 p-3 bg-green-50 rounded-lg">
                  <p className="text-xs text-green-700">✅ 予約が確定しました！ご来店をお楽しみに。</p>
                  <button onClick={() => cancelReservation(res.id)}
                    className="mt-2 text-xs text-red-500 border border-red-200 px-3 py-1 rounded">
                    予約をキャンセルする
                  </button>
        ...
        {pastReservations.length > 0 && (
          <div>
            ...
            {pastReservations.map(res => (
              <div key={res.id} className="bg-white rounded-lg shadow p-4 mb-3 opacity-75">
                <div className="flex justify-between items-start mb-2">
                  <span className={`text-xs px-2 py-1 rounded-full font-bold ${statusConfig[res.status].color}`}>
                    {statusConfig[res.status].icon} {statusConfig[res.status].label}
                  </span>
                ...
                <p className="font-bold">{res.salons?.name}</p>
                <p className="text-sm text-gray-600">{res.menus?.name}</p>
                <p className="text-sm text-pink-600 font-bold">¥{res.menus?.price?.toLocaleString()}</p>
              </div>
            ))}
          </div>
